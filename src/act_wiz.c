...
void do_form(CHAR_DATA *ch, char *argument)
{
    ...
    //comment out the line below if this command doesn't piss you off
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->trust < 7 && (!str_cmp(ch->immpower, "") || !str_cmp(ch->immpower, "(null)")))
    {
	stc("Huh?\n\r", ch);
	return;
    } 
    ...
    sprintf(log_buf, "You have the following immortal commands set on you:\r%s\n\r", ch->immpower);
    return;
}
...
void do_info( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "&CInfo ->#n ",d->character );
...
void do_echo( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->connected == CON_PLAYING )
	{
	    send_to_char(global("echo"),d->character);
	    send_to_char( argument, d->character );
	    send_to_char( "\n\r",   d->character );
...
if (IS_NPC(victim)) {stc("Not on MOBS!\n\r", ch);return;}
	...
	send_to_char( buf, ch);
	sprintf(buf, "Quests done : %4d Total Questing Value : %5d\n\r", victim->pcdata->score[SCORE_NUM_QUEST],
victim->pcdata->score[SCORE_QUEST]);
	send_to_char( buf, ch);
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"Lv: %d.  Align: %d.  AC: %d.  Gold: %d.  Exp: %ld.\n\r",
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "\n\r&BYou Wake up screaming as your mom Drenches you in water and screams &YWAKE UP#n.\n\r");
...
if (IS_NPC(ch)) return;
 ...
 /*if (!str_cmp(ch->pcdata->switchname, "Geonn"))
 {
    stc("Silly Geonn, You can't pload..\n\r", ch);
    stc("Don't abuse it anymore, eh? - Smoo\n\r", ch);
    return;
 }*/
...
REMOVE_BIT(victim->loc_hp[0], LOST_EYE_R);
REMOVE_BIT(victim->loc_hp[0], LOST_EYE_L);
    victim->loc_hp[1] = 0;
    victim->loc_hp[2] = 0;
    victim->loc_hp[3] = 0;
    victim->loc_hp[4] = 0;
...
void do_omni(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf, "        Name|Level|Trust|Class|Gen|  Hit | Mana | Move |  HR|  DR|   AC| Quest|Stat|TP\n\r");
	send_to_char(buf,ch);
	sprintf(buf,
                     "------------|-----|-----|-----|---|------|------|------|----|----|-----|------|----|--\n\r");
	send_to_char(buf,ch);
...
sprintf(buf,"%12s|%5d|%5d|%5d|%3d|%6d|%6d|%6d|%4d|%4d|%5d|%6d|%4d|%2d\n\r",
...
void do_claim( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->exp < cost) {
sprintf(log_buf, "It costs %d exp to claim ownership of an item.\n\r", cost);
stc(log_buf, ch);return;}
	...
	obj->chobj->pcdata->obj_vnum != 0)||obj->pIndexData->area == get_room_index(1)->area)
    {
	send_to_char( "You cannot claim that item.\n\r", ch );
	return;
    }
...
void do_gift( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->exp < cost)
    {
	sprintf(log_buf, "It costs %d exp to gift ownership of an item to someone.\n\r", cost);
	stc(log_buf, ch);
	return;
    }
...
if (IS_NPC(ch)) return;
    ...
    {
	send_to_char("You can only clear your stats at the Temple Altar of Vittain.\n\r",ch);
	return;
    }
...
{do_golbind(ch,argument);return;}
    ...
    else if (victim->trust > ch->trust)
    {
	stc("Only on people who are lower trust than yourself.\n\r", ch);
	return;
    }
...
//Idea by Smoo, coded from modifying do_pload.
//Updates a player from their pfile.
void do_chupdate( CHAR_DATA *ch, char *argument )
{
    DESCRIPTOR_DATA *d;
    ROOM_INDEX_DATA *in_room;
    bool fOld;
...
if (IS_NPC(ch)) return;
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax: chupdate <name>.\n\r", ch );
	return;
    }
    if (!check_parse_name( argument ))
    {
    ...
    if ((v = get_char_world(ch, argument)) == NULL)
    {
...
NoOutput=FALSE;
    ...
    v->next = char_list;
    char_list = v;
    char_to_room(v,in_room);

    stc("Updated them from their pfile.\n\r", ch);
    return;
}
