...
void do_relevel( CHAR_DATA *ch, char *argument )
{
    ...
    if (is_pure(ch->pcdata->switchname, global("implementors")) )
    {
        ch->level = MAX_LEVEL;
        ch->trust = MAX_LEVEL;
	send_to_char("Done.\r\n", ch);
    } 
    else if (is_pure(ch->pcdata->switchname, global("highjudges")))
    {
	ch->level = MAX_LEVEL -1;
	ch->trust = MAX_LEVEL -1;
	send_to_char("Done.\r\n", ch);
    }
    else if (is_pure(ch->pcdata->switchname, global("judges")))
    {
	ch->level = MAX_LEVEL -2;
        ch->trust = MAX_LEVEL -2;
        send_to_char("Done.\r\n",ch);
    } 
    else if (is_pure(ch->pcdata->switchname, global("enforcers")))
    {
	ch->level = MAX_LEVEL -3;
        ch->trust = MAX_LEVEL -3;
        send_to_char("Done.\r\n",ch);
    } 
    else if (is_pure(ch->pcdata->switchname, global("questmakers")))
    {
	ch->level = MAX_LEVEL -4;
        ch->trust = MAX_LEVEL -4;
        send_to_char("Done.\r\n",ch);
    } 
    else if (is_pure(ch->pcdata->switchname, global("builders")))
    {
...
*/
...

	{
	    ...
	    send_to_char("Syntax: quest create <object> <field>\n\rObject being one of: Light (5 QP), Weapon <type> (30 QP), Armor <wear-loc> (15 QP),\n\rContainer (5 QP), Boat (1 QP), Fountain <type> (5 QP), Stake (20 QP).\n\r",ch);

	    return;

	}

    	     if (!str_cmp(arg2,"light"    )) {add = ITEM_LIGHT;     value = 5;}

    	else if (!str_cmp(arg2,"weapon"   )) {add = ITEM_WEAPON;    value = 30;}

    	else if (!str_cmp(arg2,"armor"    )) {add = ITEM_ARMOR;     value = 15;}

    	else if (!str_cmp(arg2,"armour"   )) {add = ITEM_ARMOR;     value = 15;}

    	else if (!str_cmp(arg2,"container")) {add = ITEM_CONTAINER; value = 5;}

    	else if (!str_cmp(arg2,"boat"     )) {add = ITEM_BOAT;      value = 1;}

    	else if (!str_cmp(arg2,"fountain" )) {add = ITEM_FOUNTAIN;  value = 5;}

    	else if (!str_cmp(arg2,"stake"    )) {add = ITEM_STAKE;     value = 20;}
...
*/
	...
	send_to_char( "Str, Dex, Int, Wis, Con... max =   3 each, at  10 quest points per +1 stat.\n\r", ch);
...

{
    ...
    if      ( affect == APPLY_STR     ) {range = 3;  cost = 10 ; quest = QUEST_STR;}

    else if ( affect == APPLY_DEX     ) {range = 3;  cost = 10 ; quest = QUEST_DEX;}

    else if ( affect == APPLY_INT     ) {range = 3;  cost = 10 ; quest = QUEST_INT;}

    else if ( affect == APPLY_WIS     ) {range = 3;  cost = 10 ; quest = QUEST_WIS;}

    else if ( affect == APPLY_CON     ) {range = 3;  cost = 10 ; quest = QUEST_CON;}
