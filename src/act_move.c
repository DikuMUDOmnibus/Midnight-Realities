...
char *	const	dir_name	[]		=
{
    "&Rnorth#n", "&Geast#n", "&Ysouth#n", "&Bwest#n", "&Mup#n", "&Cdown#n"
};
...
{
    if (wall->item_type == ITEM_WALL &&
is_name(door==0?"walln":door==1?"walle":door==2?"walls":door==3?"wallw":door==4?"wallu":"walld", wall->name))
	break;
}
if (wall)
{
    stc("There is a wall of blood blocking the exit\n\r", ch);
    return;
}
...
if (!obj) {stc("The book is not open to a page..\n\r", ch);return;}
	...
	else
	{
		...
		if ( arg2[0] == '\0' ||
		    ...
		    SET_BIT(obj->spectype, ADV_FAILED);
		else if (object->pIndexData->area == get_room_index(1)->area)
		    SET_BIT(obj->spectype, ADV_FAILED);
		else if (object->pIndexData->vnum >= 8920 && object->pIndexData->vnum <= 8933)
		    SET_BIT(obj->spectype, ADV_FAILED);
...
{
    if (IS_NPC(ch) || (!IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch, CLASS_MONK) && !IS_CLASS(ch, CLASS_NINJA) && !IS_CLASS(ch, CLASS_AMAZON) && !IS_CLASS(ch, CLASS_MOOGLE)))
    {
...
{stc("You are unable to break your hands or feet free from the giant rock!\n\r", ch);
 return;}
    ...
    if (find_kingdom(ch->kingdom))
	ch->home = find_kingdom(ch->kingdom)->Gateway;
...
void train(CHAR_DATA *, char *, int);
void do_train( CHAR_DATA *ch, char *argument )
{
    train(ch,argument,1);
    return;
}
void train(CHAR_DATA *ch, char *argument, int wo)
{
    int *thestat=NULL;
    ...
    if (!str_cmp(arg2, "all")) wo = 10000000; else
    ...
    if ( arg1[0] == '\0' )
    {
	sprintf( buf, "You have %ld experience points.\n\r", srew?rand():ch->exp );
	send_to_char( buf, ch );
	strcpy(arg1,"foo");
    }
...
for (loop = 1; loop < MAX_DISCIPLINES; loop++)
  ...
  if (!str_cmp(arg1, "hp") || !str_cmp(arg1, "mana") || !str_cmp(arg1, "mv") || !str_cmp(arg1, "move"))
  {
    if (ch->tier >= 3)
    {
	stc("Tier threes may not gain hp mana nor movement points this way. Help tier3 to learn how to gain stats.\n\r", ch);
	return;
    }
    thestat = !str_cmp(arg1, "hp") ? &ch->max_hit : !str_cmp(arg1, "mana") ? &ch->max_mana : !str_cmp(arg1, "mv") || !str_cmp(arg1, "move") ? &ch->max_move : NULL;
    if (thestat == NULL) {stc("Bug.Inform an imm.\n\r", ch);return;}
#define getcost (*thestat < 10000 ? *thestat*4 : *thestat < 15000 ? *thestat*13 : *thestat < 20000 ? *thestat *26 : *thestat *38)
      while (wo)
      {int hcost = getcost;

	if (hcost < 0) hcost = 1500;
	wo--;
	if (thestat == &ch->max_mana || thestat == &ch->max_move)
	    hcost /= 4;
	else hcost /= 2;
	if (*thestat > atoi(global("max_stat"))+(ch->tier==1?0:5000))
	{
	    stc("You already have the maximum ammount!\n\r", ch);
	    return;
	}
	if (ch->exp < EXP(ch,hcost))
	{
	    sprintf(log_buf, "&gYou do not have the &Y%d&g exp required.\n\r", EXP(ch,hcost));
	    stc(log_buf, ch);
	    return;
	}
	(*thestat)++;
  ...
  if ( arg2[0] != '\0' && is_ok )
	...
	if (amount < 1 || amount > 50)
	{
	    ...
	    stc("Train needs to be recoded before I can increase this amount. Please be patient.\n\r", ch);
	    return;
	}

    if (amount > 400) {stc("You may only train 400 of any particular stat at a time, until multiple trains are recoded.\n\r", ch);
		   return;}
    ...
    primal = (1+ch->practice)*3500;
    magic = (1+ch->pcdata->stats[DROW_MAGIC])*800;
    silver = (1+ch->siltol)*2500;
    ancilla = 50000000;
    elder = 100000000;
    methuselah = 250000000;
    ancilla = EXP(ch,ancilla);
    elder = EXP(ch,elder);
    methuselah = EXP(ch,methuselah);
    silver = EXP(ch, silver);
    if (IS_CLASS(ch, CLASS_HIGHLANDER)) max_stat = 25;
    if (ch->tier == 3) max_stat = 50-IS_CLASS(ch, CLASS_HIGHLANDER)?0:18;
	...
	if (ch->class & CLASS_HIGHLANDER)
	    stc("Highlanders get their stats cleared when they train avatar. This is to solve a bug that can give them hitpoints. Please re-wear your equipment.\n\r", ch);
	else if (ch->class & CLASS_MONK)
	    stc("Monks get their stats cleared when they train avatar. This is so that they can't get around the not-wearing-weapons thing as a monk. (Monks can't wear weapons, but they can as a mortal) Please re-wear your equipment.\n\r", ch);
	}
	ch->level++;
	sprintf(log_buf, "&R%s &Ghas trained &BAvatar&G.&w", ch->pcdata->switchname);
	do_info(ch, log_buf);
	if (ch->trust < 7 && ch->trust > 2) ch->level = ch->trust;
	return;
    }
    ...
    else if ( !str_cmp( arg1, "primal") && ch->practice < 4999)
    {
        cost        = primal;
	...
	pOutput     = "primal";
		...
		ch->pcdata->stats[DROW_MAGIC] < 80 && ch->tier <=2)
	{
	...
	pOutput = "drow magic resistance";}
		...
		ch->pcdata->stats[DROW_MAGIC] < 90 && ch->tier >=3)
	{
	sh_int quiet_pointer=ch->pcdata->stats[DROW_MAGIC];
	cost = magic;
	pAbility = &quiet_pointer;
...
{    
    ...
    if (ch->exp < ancilla)
    {
	sprintf(log_buf,"You need %d exp to become ancilla!\n\r", ancilla);
	stc(log_buf, ch);
       return;
    }
      ch->exp -= ancilla;
   ...
   (ch->pcdata->rank == AGE_ANCILLA))
      {
        if (ch->exp < elder)
        {
	sprintf(log_buf, "You need %d exp to become an elder.\n\r", elder);
	stc(log_buf, ch);
         return; 
        }
        ch->exp -= elder;
...
&& (ch->pcdata->rank == AGE_ELDER))
  {
     if (ch->exp < methuselah)
      {
	sprintf(log_buf, "&wYou need &R%ld &RMORE&wexp to become Methuselah!\n\r", methuselah - ch->exp);
	stc(log_buf, ch);
      return;
      }
    ch->exp -= methuselah;
...
if (ch->tier == 3)
    stc("You may not train hp, mana, nor movement points because you are tier III. Help tier3 to learn how to gain stats.\n\r", ch);
else
{
    if ( ch->max_hit < atoi(global("max_stat")))
	{
		thestat = &ch->max_hit;
		sprintf (buf,"Hp                - %d exp per point.\n\r", getcost/2 );
		send_to_char(buf,ch);
	}
	if ( ch->max_mana      < atoi(global("max_stat")))
	{
	    thestat = &ch->max_mana;
	    sprintf( buf, "Mana             - %d exp per point.\n\r", getcost/4 );
	   send_to_char( buf, ch );
	}
	if ( ch->max_move      < atoi(global("max_stat")))
	{
	   thestat = &ch->max_move;
	...
	{
	 if (ch->tier < 3 && ch->siltol >= 75)
	 {
	    stc("You have maximum silver tolerance for your tier: 75.\n\r", ch);
	 } else {
	   sprintf( buf, "Silver tolerance - %d exp per point of tolerance.\n\r", silver );
	   send_to_char( buf, ch );}
	}
	if (ch->pcdata->stats[DROW_MAGIC] <= 80 && IS_CLASS(ch, CLASS_DROW))
	{
	if (ch->pcdata->stats[DROW_MAGIC]==80 && ch->tier <=2)
	  stc("You have maximum magic resistance for your tier: 80\n\r", ch);
	else if (ch->pcdata->stats[DROW_MAGIC]==90)
	  stc("You have maximum magic resistance for your tier: 90\n\r", ch);
	else {
    ...
    if ( (*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1, "str")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1, "int")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1, "wis")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1, "dex")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if  ((*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1,
"constitution")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if  ((*(sh_int*)pAbility >= max_stat) && (!str_cmp( arg1, "con")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*(int*)pAbility >= 4999) && (!str_cmp( arg1, "primal")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }
    if ( (*(int*)pAbility >= 100) && (!str_cmp( arg1, "silver")))
    {
	if (last) act( "Your $T is already at maximum.", ch, NULL, pOutput, TO_CHAR );
	return;
    }

if (str_cmp(arg1, "magic")) cost = EXP(ch, cost);
    if ( cost < 1 ) cost = 1;
...
{
    char arg1[MIL];
    int hit=ch->max_hit;int mana=ch->max_mana;int move=ch->max_move;
    unsigned long exp=ch->exp;
    int times, track=0;

    argument = one_argument(argument, arg1);
    if (!*argument || (str_cmp(arg1, "hp") && str_cmp(arg1, "mana") && str_cmp(arg1, "move") && str_cmp(arg1, "mv")) || !is_number(argument))
    {
	stc("Syntax: estimate hp/mana/mv (how much you want to gain)\n\r", ch);
	return;
    }
    for (times=atoi(argument), ch->exp = 999000000;times;times--,ch->exp = 999000000)
    {
	train(ch,arg1,1);
	if (ch->exp==999000000)
	{
	    stc("You tried to go over the limit.\n\r", ch);
	    break;
	}
	track += (999000000-ch->exp);
    }

    ch->exp=exp;ch->max_hit=hit;ch->max_mana=mana;ch->max_move=move;
    sprintf(log_buf, "&GIt took you &Y%u&G exp to train &Y%d&G points of &R%s&G.\n\r", track, atoi(argument)-times, arg1);
    stc(log_buf, ch);
return;
}
...
void do_tie( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if (victim->generation > 1 && victim->generation <= ch->generation && (ch->generation - victim->generation) <= 2)
	{int a = ch->generation;
	...
	if ((IS_CLASS(ch, CLASS_DEMON)) && (IS_CLASS(victim, CLASS_DEMON)) )
	{
		if ( (victim->generation == 13) && (ch->generation == 12) && (!str_cmp(victim->clan, ch->clan)))
		{
			...
			ch->generation = 13;
			sprintf(buf,"%s is now a Pit Fiend.",ch->name);
			do_info(ch,buf);
		}
	}
/*	else if (!IS_NPC(victim) && !IS_NPC(ch) && ch->class == victim->class && ch->generation > victim->generation &&
(!str_cmp(ch->clan, victim->clan)||victim->generation ==1) && !IS_CLASS(ch, CLASS_GOLEM))
	{
		int sw = ch->generation;
		ch->generation = victim->generation;
		victim->generation = sw;
		sprintf(log_buf, "%s absorbs %s's generation.", ch->pcdata->switchname, victim->pcdata->switchname);
		do_info(ch,log_buf);
	}*/
    return;
}
...
void do_texturize(CHAR_DATA *ch, char *argument)
{
 ...
 if (IS_NPC(vict))
 {
    stc("Sorry, but monsters are already set in stone.\n\r", ch);
    return;
 }
 ...
 vict->level = 3;
 vict->exp = 0;
 stc("&GYou have been stripped of your exp!\n\r", vict);
