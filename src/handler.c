...
//   CHAR_DATA*pt;
    ...
    if (pRoomIndex->vnum == 8 && !IS_CLASS(ch,CLASS_GOLEM) && ch->trust < 7)
    {
	stc("Get out of that room!\n\r", ch);
	char_to_room(ch,get_room_index(3001));
	return;
    }

    if (pRoomIndex->vnum != 8 && IS_CLASS(ch,CLASS_GOLEM) && ch->pcdata->powers[2] != 0)
	ch->pcdata->powers[2] = 0;

    if (pRoomIndex->vnum == 9 && ch->trust < 7)
    {
	stc("Get out of that room!\n\r", ch);
	char_to_room(ch,get_room_index(3001));
	return;
...
if (IS_CLASS(ch,CLASS_GOLEM) && pRoomIndex->vnum == 8 && ch->pcdata->powers[2] == 0)
...

{
    ...
    if ( obj->wear_loc != WEAR_NONE && obj->carried_by != NULL)
...

{
    ...
    if (IS_CLASS(ch, CLASS_UNICORN) && !IS_SET(obj->spectype, SITEM_HIGHLANDER))
    {
	obj_from_char(obj);
	obj_to_char(obj, ch);
	stc("That is too big for your body!\n\r", ch);
	return;
    }
    ...
    || (obj->pIndexData->vnum >= 11400 && obj->pIndexData->vnum < 11414 && !(ch->class & CLASS_GOLEM)))
    ...
    if (obj->ClassBit && !(ch->class & obj->ClassBit))
    {
	act("$n is overpowered by $p.", ch, obj, NULL, TO_ROOM);
	act("You are overpowered by $p, and drop it.", ch, obj, NULL, TO_CHAR);
	obj_from_char(obj);
	obj_to_room(obj, ch->in_room);
	save_char_obj(ch);
	return;
...

	{
		...
		if (IS_NPC(ch) || !(ch->class & CLASS_DROW))
	...
	{
		if ((!IS_NPC(ch) && !(ch->class & CLASS_JEDI)) && ((obj->pIndexData->vnum >= 29709) && (obj->pIndexData->vnum <= 29712)) )
		{
	    ...
	    if ((!IS_NPC(ch) && !(ch->class & CLASS_NINJA)) && ((obj->pIndexData->vnum >= 29670) && (obj->pIndexData->vnum <= 29673)))
...
/* End */
	    ...
	    if ((!IS_NPC(ch) && !(ch->class & CLASS_WEREWOLF)) && (((obj->pIndexData->vnum >= 29696) 
		...
		if (!IS_NPC(ch) && !(ch->class & CLASS_VAMPIRE) && !IS_SET(ch->pcdata->stats[UNI_AFF], VAM_MORTAL) 
	...
	/* Mage eq drop Adreno */
	    if ((!IS_NPC(ch) && !(ch->class & CLASS_MAGE)) && ((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)))
...
/* End */
	...
	/* Monk eq drop Adreno */
	    if ((!IS_NPC(ch) && !(ch->class & CLASS_MONK)) && ((obj->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)))
...
/* amazon eq drop Adreno  */
	    if ((!IS_NPC(ch) && (!(ch->class & CLASS_AMAZON)) && ch->class != 256) &&
...
/*	    if (((IS_NPC(ch) || !(ch->class & CLASS_DEMON)) && IS_SET(obj->spectype, SITEM_DEMONIC)) && !((obj->pIndexData->vnum >=
29670) && (obj->pIndexData->vnum <= 29673)) && !((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)) && !((obj
->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)) && !((obj->pIndexData->vnum >= 27943) && (obj->pIndexData->vnum <= 27948)))*/
		if (!IS_NPC(ch) && (!(ch->class & CLASS_DEMON)) && ( ((obj->pIndexData->vnum >= 27931) && (obj->pIndexData->vnum <=
27942)) || ((obj->pIndexData->vnum >= 29650) && (obj->pIndexData->vnum <= 29663)) ) )
		{
...
/*		if (((IS_NPC(ch) || !IS_IMMORTAL(ch)) && IS_SET(obj->quest,QUEST_RELIC) && !IS_SET(obj->spectype,SITEM_DEMONIC) &&
!IS_SET(obj->spectype,SITEM_HIGHLANDER) && !(ch->class & CLASS_WEREWOLF) &&  obj->chobj == NULL) && !((obj->pIndexData->vnum >= 29670) &
 ...
 */ // reenabled - Midnight Realities 03/03/01
    ...
    if (((IS_NPC(ch) || !(ch->class & CLASS_HIGHLANDER)) && IS_SET(obj->spectype, SITEM_HIGHLANDER)) && !((obj->pIndexData->vnum >=
29670) && (obj->pIndexData->vnum <= 29673)) && !((obj->pIndexData->vnum >= 29699) && (obj->pIndexData->vnum <= 29701)) && !
((obj->pIndexData->vnum >= 29706) && (obj->pIndexData->vnum <= 29708)))
    ...
    if ( IS_NPC(ch) || !(ch->class & CLASS_HIGHLANDER) ) 
	...
	if (obj->spectype & SITEM_WOODEN)
	{
	    if (!IS_SET(ch->itemaffect, ITEMA_WOODEN))
	    {
		SET_BIT(ch->itemaffect, ITEMA_WOODEN);
		act("&yThe wood feels like silk on $n's hands.",ch,0,0,TO_ROOM);
		stc("The wood feels like silk in your hands.\n\r", ch);
	    }
	}
...

{
	...
	if (obj->spectype & SITEM_WOODEN)
	{
	    if (IS_SET(ch->itemaffect, ITEMA_WOODEN))
	    {
		REMOVE_BIT(ch->itemaffect, ITEMA_WOODEN);
		stc("Blisters form on your hands.\n\r", ch);
		act("Blisters form on $N's hands.", ch,0,0,TO_ROOM);
	    }
	}

	if (IS_AFFECTED(ch, AFF_PROTECT) && (sn == 54 || sn == 7)
...

{
	...
	bug( "obj_from_room: NULL. vnum d", obj->pIndexData->vnum );
...
if (!str_cmp(argument, "self")) return ch;
    ...
    if ( (number = number_argument(argument, arg)) == 1
...
switch (wch->in_room->vnum)
{
    ...
    case 11392:
    case 11393:
	if (!IS_NPC(ch) && ch->level >= 7) break; else
	continue;
    default:
}
    if (!IS_NPC(wch) && wch->pcdata->chobj) continue;

    c1 = FALSE;
    strcpy(buf2, arg);
    p = buf2;

    for (p = one_argument(p, buf); buf[0] && !c1; p = one_argument(p, buf))
    {
	...
	else if ( !can_see( ch, wch ) || ( !is_name( buf, wch->name ) &&

	    ( IS_NPC(wch) || (!is_name( buf, wch->morph )))))
...

{
    ...
    if (pRoomIndex->vnum==9) return FALSE; // room 9 is used for GUMMY BEARS. Removing this will put the mud in a loop
...
if(find_kingdom(ch->kingdom)&&find_kingdom(ch->kingdom)==find_kingdom(victim->kingdom)&&is_pure("ksight",find_kingdom(ch->kingdom)->Skills)&&ch->trust<7&&victim->trust<7)
    return TRUE;

if (is_pure("holylight",ch->immpower)) return TRUE;

if (IS_CLASS(ch, CLASS_UNICORN) && !IS_IMMORTAL(victim)) return TRUE; // unicorns are 
// next-to-immortal, believed to have created the earth creatures for the immortals 
// (they were too lazy)

if (IS_SET(victim->extra, EXTRA_EARTHMELD) && !IS_IMMORTAL(ch))
    return FALSE;

if (ch->spec_fun == spec_lookup("spec_eater")) return TRUE;

if (IS_ITEMAFF(ch,ITEMA_VISION) && victim->level < 7 && !IS_CLASS(ch,CLASS_MAGE)) return TRUE;
...
if (victim->act & PLR_SACREDINVIS && IS_CLASS(victim, CLASS_JEDI))
{
    if ((victim->generation/2)+1 < ch->generation)
	return FALSE;
}
...
IS_SET(victim->act, PLR_WIZINVIS)) return FALSE;
   ...
   if (is_affected(ch, skill_lookup("phoenix sight"))) return TRUE;
