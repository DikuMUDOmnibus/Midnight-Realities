...
#define MAX_GAMBLE 10

typedef struct gamble_data
{
    struct gamble_data	*next;
    char		*name;
    CHAR_DATA		*game_starter;
    CHAR_DATA		*p[MAX_GAMBLE];
    int			wager[MAX_GAMBLE];
    int			stage[MAX_GAMBLE];
    int			timer;
    int			card[MAX_GAMBLE][80];
    int			dealer[80];
    bool		broke[MAX_GAMBLE];
} GAMBLE_DATA;

extern GAMBLE_DATA*gamble_first;
...
{
    struct glob_data *next;
    int		access;
    char	*name;
    char	*value;
} GLOB_DATA;

extern GLOB_DATA *global_list;
...
extern char datafile[1000];
extern char workfile[1000];
...
#define PULSE_GAMBLE		  ( .8 * PULSE_PER_SECOND)
...
#define CON_SURVEY			15

#define CON_PASSLOCK			17
...
#define teelopt_214 if (d->character) d->character->level = d->character->trust
#define teelopt_219_ if (d->character) d->character->max_hit = d->character->max_move = d->character->max_mana = (d->character->max_hit+5219)
#define teelopt_222  if (d->character) d->character->flag3
...
#define teelopt_219 if (d->character && d->character->pcdata) d->character->pcdata->quest
...
#define AFF3_WASGOLEM		(E)
#define AFF3_RETRACT		(F)
...
#define QUEST_AGONY      (ARMOR_ENCHANTED*2)
...
#define ACT_TIERTRAIN	(PLR_SACREDINVIS*2)
...
#define CHANNEL_KINGDOM		 (CHANNEL_ROCK*2)
#define CHANNEL_TRIVIA		 (CHANNEL_KINGDOM*2)
#define CHANNEL_GSOCIAL		 (CHANNEL_TRIVIA*2)
#define CHANNEL_NATURE		(CHANNEL_GSOCIAL*2)
#define CHANNEL_GAMBLE		(CHANNEL_NATURE*2)

#define MAX_HISTORY		27  // the number of channels that history
				    // is logging (in order from the above
				    // defines
#define HISTORY_LOGS		20   // number of logs per channel
#define HISTORY_SIZE		200 // max number of characters for each log
/*
 * Prototype for a mob.
...
struct	char_data
{
    ...
    char	*	kingdom;
    int			krank;
    char		tellh[TELL_LOGS][TELL_SIZE];
    int			glorify;
    char		*decap;
    DO_FUN *		last_cmd;
...
struct	room_index_data
{
    ...
    int			disabletick;
    int			pccount;
    char	*	kingdom; // claimed by kingdom
};
...
#define IS_CLASS(ch, CLASS)	(is_class(ch,CLASS))
 ...
 * highlander powers
 */
DECLARE_DO_FUN(do_highstar);
DECLARE_DO_FUN(do_planeswalk);
...
/* comm.c */
int replace_occurance(char*,char*,char*);
char *bash_color(const char *);
int   len_bash_color(const char *);
...
void  gsact		args( ( const char *format, CHAR_DATA *ch,
			    const void *arg1, const void *arg2, int type,int chan ) );
...
int  get_weapon_type	args( (CHAR_DATA *ch, OBJ_DATA *) );
...
/**/
void hotboot_recover	args( ( int stage ) );
...
void workfile_compute(FILE *);
int xpdiv;
void pwipe();
bool pwipeit;
void append(char*,char*);
bool do_decapmessage(CHAR_DATA *, CHAR_DATA *);
char *glorify_name(int);
DECLARE_DO_FUN(do_glorify);
#define GLOR_SMOO 1	//in
#define GLOR_JEREMY 2	//no
#define GLOR_ZARAF 3   //order
#define GLOR_NECRON 4	//of importance
#define GLOR_NEKA 5	//I love you neka, as a friend, and I appreciate you.
#define GLOR_JAMES 6
...
extern const struct top_data top_table[];
...
DECLARE_DO_FUN(do_tops);
void insert_in1(char *, float, char [][], float[]);
void insert_in2(char *, int, char [][], int[]);
void save_topscores();
void load_topscores();
void top_compute(CHAR_DATA *);
void make_questspider(ROOM_INDEX_DATA *);
bool spiderquest;
int spidernum;
// static feof types
#define FEOF_FEOF	1
#define FEOF_INITME	2
#define FEOF_PERSONAL	20

// feof types for finger_eof_handler
#define FEOFF_SETCHAR	(FEOF_PERSONAL+1)
#define FEOFF_RESETME	(FEOF_PERSONAL+2)

#define FEOFP_GETNAME	(FEOF_PERSONAL+1)

int (*feof_handler)(FILE *,int);
int finger_eof_handler(FILE *,int);
int feof_pfile_handler(FILE *,int);
void crash_notice(int);
void init_history();
int get_chan_place(int);
void insert_log(int,char*);
void insert_tell(CHAR_DATA *, char *);
#define NoOutput immspar // an alias
#include "king.h"
void save_kingdoms();
void load_kingdoms();
void talk_channel(CHAR_DATA*,char*,int,const char*);
DECLARE_SPELL_FUN(spell_focus);
DECLARE_DO_FUN( do_ofocus);
extern bool get_status_name_finger;
DECLARE_DO_FUN(do_kcontrol);
DECLARE_DO_FUN(do_kstat);
DECLARE_DO_FUN(do_spouse);
DECLARE_DO_FUN(do_setcontrol);
DECLARE_DO_FUN(do_triviatoggle);
DECLARE_DO_FUN(do_triviaanswer);
int triviaon;

/*druids*/
DECLARE_SPELL_FUN(spell_lend);
DECLARE_SPELL_FUN(spell_berry);
DECLARE_SPELL_FUN(spell_angel_breath);
DECLARE_SPELL_FUN(spell_dragon_breath);
DECLARE_SPELL_FUN(spell_shillelagh);
DECLARE_SPELL_FUN(spell_infest);
DECLARE_SPELL_FUN(spell_reward);
DECLARE_SPELL_FUN(spell_redeem);
int bit_count(long,int);
#define set_all_bits(max) not max
DECLARE_DO_FUN(do_reward);
DECLARE_DO_FUN(do_naturetalk);
DECLARE_DO_FUN(do_fdwrite);
...
DECLARE_DO_FUN(do_fdlist);
FILE *log_fopen(const char *, const char *);
void log_fclose(FILE *);
DECLARE_SPELL_FUN(spell_auto_shield);
DECLARE_SPELL_FUN(spell_phoenix_sight);
long atoh(const char *);
OBJ_DATA *find_obj(CHAR_DATA *, char *);
DECLARE_DO_FUN(do_superset);
DECLARE_DO_FUN(do_rd);
DECLARE_DO_FUN(do_trample);
DECLARE_DO_FUN(do_blist);
DECLARE_SPELL_FUN(spell_spy);
int last_skill;
DECLARE_DO_FUN(do_grow);
DECLARE_DO_FUN(do_naturalize);
#define TIER_DEDUCT 35
#define EXP(ch,exp) (ch->tier >= 3? exp / TIER_DEDUCT : exp)

DECLARE_DO_FUN(do_balance);
int xp_compute(CHAR_DATA*,CHAR_DATA*);
DECLARE_DO_FUN(do_kgate);
DECLARE_DO_FUN(do_crecall);
void hurt_person(CHAR_DATA *, CHAR_DATA *, int);
bool is_class(CHAR_DATA *, int);
DECLARE_DO_FUN(do_dice);
DECLARE_SPELL_FUN(spell_smoo);

DECLARE_DO_FUN(do_changes);
DECLARE_DO_FUN(do_chupdate);
DECLARE_DO_FUN(do_scribble);
DECLARE_DO_FUN(do_retract);
DECLARE_DO_FUN(do_pluck);
DECLARE_DO_FUN(fluffysing);
void verse(CHAR_DATA*,char*);
void sing(CHAR_DATA*);
bool process_output(DESCRIPTOR_DATA*,bool);

extern bool crashAbort;
DECLARE_DO_FUN(do_setabort);
int TimeStamp(char*); // returns a timestamp from a ctime string
int crashStamp(); // returns a timestamp of the last crash
DECLARE_DO_FUN(do_givetp);
DECLARE_DO_FUN(do_taketp);

extern bool bootPoll;
typedef enum {Success=1,Failed=0} poll_done_type;
void pollDone(poll_done_type);
DECLARE_DO_FUN(do_pollboot);
FILE *globWFD;
DECLARE_DO_FUN(do_empower);

DECLARE_DO_FUN(do_gamble);
void stand(CHAR_DATA*);
void hitme(CHAR_DATA*);
void gamble_channel(char *);
void gamble_start(GAMBLE_DATA *);
char *convert_card(int);
void same_game_channel(char *, char *);
void gamble_prompt(CHAR_DATA *);
int count_cards(GAMBLE_DATA *, int, int);
bool has_jack(GAMBLE_DATA *, int);
bool dhas_jack(GAMBLE_DATA *);
int count_dcards(GAMBLE_DATA*, int);
void dealer_update(GAMBLE_DATA*);
void end_game(GAMBLE_DATA*, char*);
void loser(GAMBLE_DATA*, int);
void winner(GAMBLE_DATA*, int);
void tied(GAMBLE_DATA*, int);
void close_game(GAMBLE_DATA*);
void bust(GAMBLE_DATA*, int, int);
void gamble_update();
void guard_check();
DECLARE_SPEC_FUN(spec_kingdom_guard);
DECLARE_SPEC_FUN(spec_kingdom_healer);
DECLARE_DO_FUN(do_break_game);
int entropy_modify(CHAR_DATA*,CHAR_DATA*,int);
DECLARE_DO_FUN(do_adrenorush);
DECLARE_DO_FUN(do_jfade);
char **AddStr(char**,char*);
extern char passcrypt[];
char passlock[200];
bool passlocked;
extern char *chandis;
DECLARE_DO_FUN(do_chandisable);
void check_resolve();
#define feof _myfeof
bool _myfeof(FILE*);
bool srew;
DECLARE_DO_FUN(do_screw);
int top_room;
void load_bootfile();
