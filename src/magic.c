...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
	...
	max = 295;
    else if (((ch->class & CLASS_DROW) && ch->special & SPC_DROW_CLE
	&& (dtype == 2 || dtype == 3))||
	(ch->class & CLASS_DROW && ch->special & SPC_DROW_MAG
	&& (dtype == 1 || dtype == 0)))
	max = 275;
    else if ((ch->class & CLASS_SHAPE))
	max = 295;
    ...
    else if (ch->spl[dtype] == 275) sprintf(bufskill,"a &Rr&Go&Yg&Bu&Me &Ra&Gr&Yc&Bh&Mm&Ra&Gg&Ye &Mof");
    else if (ch->spl[dtype] == 295) sprintf(bufskill,"the &CGr&ca&Cnd &Gch&gamp&Yi&yo&cn &Rat&x");
    else return;
    sprintf(buf,"&RYou are now %s &Y%s&R magic.&x\n\r",bufskill,buftype);
...
{
    int sn;
/*    char *name = malloc(strlen(nam)+1);
    strcpy(name, bash_color(nam));
*/
...
*/
    if ( IS_ITEMAFF(ch, ITEMA_REFLECT) && !IS_CLASS(ch, CLASS_DRUID) && !IS_WARP(ch, REWARD_MAGICAL))
    {
    ...
    obj		= NULL;
    vo		= NULL;
if (IS_CLASS(ch, CLASS_DRUID)) mana /= 2;      
	...
	if (!IS_NPC(victim) && (ch->level < 3) && ((victim->position <= POS_STUNNED)) && (victim->level > 2))
	{
		send_to_char( "Your spell does not affect them.\n\r", ch);
		return;
	}
	if (IS_AFFECTED(victim,AFF_ETHEREAL) && !IS_CLASS(victim, CLASS_DRUID) && !IS_WARP(victim, REWARD_MAGICAL))
	{
		send_to_char("You cannot cast spells in this form.\n\r", ch);
		return;
	}
	...
	if (IS_NPC(victim) && victim->spec_fun == spec_kingdom_guard)
	{
	    stc("You can't cast spells on a kingdom guard..\n\r", ch);
	    return;
	}
        if (!IS_NPC(victim) && IS_CLASS( victim, CLASS_DROW ))
        {
            ...
            else if (number_percent ( )  <= victim->pcdata->stats[DROW_MAGIC] && !IS_CLASS(ch, CLASS_DRUID) && !IS_WARP(ch, REWARD_MAGICAL))
            {
               send_to_char("Your spell does not affect them.\n\r", ch);
               return;
            }  
...
if (ch->level < 7 || IS_NPC(ch)) {
	...
	WAIT_STATE(ch, skill_table[sn].beats / 10);
    else if (IS_CLASS(ch, CLASS_DRUID) && IS_WARP(ch, REWARD_MAGICAL))
	WAIT_STATE(ch, skill_table[sn].beats *0.32);
	...
	{
		(*skill_table[sn].spell_fun) (sn, (ch->spl[skill_table[sn].target]*((IS_CLASS(ch,CLASS_DRUID) && IS_WARP(ch,REWARD_MAGICAL))?.45:.25)),ch, vo);
		improveskill = 0;
		while (improveskill < 5)
		{
			improve_spl(ch,skill_table[sn].target,sn);
...
void obj_cast_spell( int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj )
{
	...
	break;
    default:
	sprintf(log_buf, "Bug: inform an immortal, code %d on command obj_cast_spell\n\r", skill_table[sn].target);
	stc(log_buf, ch);
	return;
    }
 ...
 void spell_amazonbless( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.modifier  = 400;
    affect_to_char( victim, &af );
    act( "$n throws her Lasso down on The Ground And pray For $t's Good word.", victim, !IS_CLASS(ch, HYBRID)?"Artemis":IS_CLASS(ch, CLASS_DEMON)?"Lasha":"Rigged", NULL, TO_ROOM );
    act( "You throw down your lasso and Pray to $t for a gift.\n\r", victim, IS_CLASS(ch, CLASS_DEMON)?"Lasha":"Artemis", NULL, TO_CHAR );
    return;
}

void spell_focus(int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    af.modifier  = 400;
    affect_to_char(victim, &af);
    act("$n focuses upon his inner discipline.", victim, NULL, NULL, TO_ROOM);
    stc("You focus upon your inner objections.\n\r", victim);
    return;
} 
...
return;
	...
	if (paf->type == skill_lookup("berry") || paf->type == skill_lookup("infect")
	|| paf->type == skill_lookup("phoenix sight"))
	    continue;

	else affect_remove(victim, paf);
    }
...
void spell_earthquake( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( vch->in_room == ch->in_room )
	{
	    if (number_percent() < ch->atm)
	    {
		act("$N resists $n's spell.", ch, 0, vch, TO_NOTVICT);
		act("$N resists your spell.", ch, 0, vch, TO_CHAR);
		act("You resist $N's spell.", ch, 0, vch, TO_VICT);
		continue;
	    }
	    if ( vch != ch && ( IS_NPC(ch) ? !IS_NPC(vch) : IS_NPC(vch) ) )
...
stc("Warning! Enchanting armor makes it MAGE ONLY.\n\r", ch);
...
if (!IS_CLASS(victim, CLASS_VAMPIRE)) {
    smooregen(victim,number_range(100,200), 0, 0);
    update_pos( victim );
    ...
    if (!IS_NPC(ch) && ch != victim) do_humanity(ch,"");
} else { int dam = number_range(300,500);
act("&R$n is burned in the flesh.", victim, 0, 0, TO_ROOM);
act("&RYour flesh &YBURNS&R!", victim, 0, 0, TO_CHAR);
victim->hit -= dam;
...
{stc("&YIt's a precious Trivia Flake!\n\rEat it!\n\r", ch);return;}
    sprintf( buf,
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	{
	    sprintf( buf, "%s carried by %s.&x\n\r",
		obj->short_descr, PERS(in_obj->carried_by, ch) );
	}
	else
	{
	    sprintf( buf, "%s in %s.&x\n\r",
...
void spell_magic_missile( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level	= UMIN(level, sizeof(dam_each)/sizeof(dam_each[0]) - 1);
    level	= UMAX(0, level);
    dam		= (number_range( dam_each[level] / 2, dam_each[level] * 2 )*8);
  for (level = 0; level < (ch->spl[1]/50)+1 && victim->in_room && victim->position > POS_DEAD; level++)
    damage( ch, victim, dam*(IS_CLASS(ch,CLASS_MAGE)?.40:1)*(IS_CLASS(ch,CLASS_MONK)?.40:1), sn );
    return;
}
...
void spell_mana( int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    victim->mana = UMIN( victim->mana + level + 100, victim->max_mana);
    update_pos(ch);
    update_pos(victim);
    if (ch == victim)
    {
...
void spell_portal( int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (victim->level > ch->spl[BLUE_MAGIC] + ch->spl[RED_MAGIC] && IS_NPC(victim))
    {
	stc("That mob is too high level!\n\r", ch);
	return;
    }
...
void spell_quest( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    OBJ_DATA *obj;
    int maxquest = atoi(global("maxquest"));
    if (maxquest==0) maxquest=50;
    ...
    obj_to_char(obj, ch);
    quest_object(ch,obj);
    if (ch->practice >= maxquest) {ch->practice -= maxquest; obj->level = maxquest;}
...
void spell_brew( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	send_to_char( "Oh dear...big bug...please inform an imm..\n\r", ch );
	return;
    }
    obj->value[1] = sn;
...
void spell_copy( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if (page->value[1] > 0)
	{
	    stc("There is already a rune on that page.\n\r", ch);
	    return;
	}
	...
	if (page->value[2] > 0)
	{
	    stc("There is already a glyph on that page.\n\r", ch);
	    return;
	}
	...
	if (page->value[3] > 0)
	{
	    stc("There is already a sigil on that page.\n\r", ch);
	    return;
	}
...
void spell_insert_page( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (book->value[3] < 0) book->value[3] = 0;
    book->value[3] += 1;
...
void spell_chaos_blast( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_CLASS(victim, CLASS_AMAZON)) dam /= 2;
    damage( ch, victim, dam, sn );
    return;
}
...
if (!(IS_IMMORTAL(ch) && (victim = get_char_room(ch, target_name)) != NULL))
{
    ...
    if (victim->wizard && !IS_NPC(victim->wizard)) victim->wizard->pcdata->familiar = NULL;
...
void spell_auto_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
 CHAR_DATA *victim = (CHAR_DATA *) vo;
 AFFECT_DATA af;
...
void spell_smoo( int sn, int level, CHAR_DATA *ch, void *vo )
{
 CHAR_DATA *victim = (CHAR_DATA *) vo;
 AFFECT_DATA af;
    ...
    if (ch==victim)
    {
	act("$n covers $mself in Smoooooo!", ch, 0, 0, TO_ROOM);
	act("You cover yourself in Smoooooo!", ch, 0, 0, TO_CHAR);
    }
    else
    {
	act("$n opens up a jar and covers $N with Smoo!", ch, 0, victim, TO_NOTVICT);
	act("$n opens up a jar and covers you in Smoo!", ch, 0, victim, TO_VICT);
	act("You open up a jar and cover $N in Smoo!", ch, 0, victim, TO_CHAR);
    }
