...
if (obj->pIndexData->vnum == 24 && !IS_CLASS(ch, CLASS_GOLEM))
{stc("Only golems and carry those precious stones.\n\r", ch);
return;}
...
void do_eat( CHAR_DATA *ch, char *argument )
{
    ...
    if (obj->miscbit[1] > 0 && !(obj->miscbit[1] & ch->class))
    {
	sprintf(log_buf, "error code %d\n\r", obj->miscbit[1]);
	stc(log_buf, ch);
	stc("You can't stomach it.\n\r", ch);
	return;
    }
...
if (obj->pIndexData->vnum == 28)
{
    ...
    case ITEM_FOOD:
	if (obj->miscbit[0]==0&&obj->miscbit[1]&CLASS_DRUID&&!ch->fighting&&!ch->fight_timer)
	    stc("It's all old and moldy...They don't save on copyovers/relogs so that you don't spam create a million..\n\r",ch);
	if (obj->miscbit[0] != 0 && obj->miscbit[1] & CLASS_DRUID && !ch->fighting && !ch->fight_timer)
	    smooregen(ch,obj->miscbit[0],0,0);
	if ( !IS_NPC(ch) )
	{
	    int condition;
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    if (IS_SLAIYTH(ch))
    {
	if (obj->wear_flags & ITEM_WIELD || obj->wear_flags & ITEM_HOLD)
	{
	    stc("Your hands are not free.\n\r", ch);
	    return;
	}
    }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    int expgain;
if (!argument) return; // wierd bug where objects will have NULL in everything, including ->name, ->short_descr
			// and tons of other stuff, do_sacrifice(ch,object->name); and stuff similar from spec_rogue
			// will crash without this.
    one_argument( argument, arg );
if (!str_cmp(arg, "all")) {do_tap(ch, "all");return;}
    if ( arg[0] == '\0' )
    {
    ...
    {
	sprintf( buf, "You receive a refund of %d demon points from $p.", obj->cost);
	act( buf, ch, obj, NULL, TO_CHAR );
	ch->pcdata->stats[DEMON_CURRENT] += obj->cost;
	ch->pcdata->stats[DEMON_TOTAL] += obj->cost;
    }
    if (obj->value[0] > 0 && !IS_NPC(ch) && obj->item_type != ITEM_PAGE 
        && IS_CLASS(ch, CLASS_DROW) && IS_SET(obj->spectype, SITEM_DROW) )
    {
	if (obj->cost == 0) obj->cost = 7500;
	sprintf( buf, "You receive a refund of %d drow points from $p.", obj->cost);
	act( buf, ch, obj, NULL, TO_CHAR );
	ch->pcdata->stats[DROW_POWER] += obj->cost;
    }
    extract_obj( obj );
    return;
}
...
void do_activate( CHAR_DATA *ch, char *argument )
{
	...
	if (CAN_WEAR(obj, ITEM_TAKE) && (obj->quest & QUEST_ARTIFACT))
	{
...
void do_recharge( CHAR_DATA *ch, char *argument )
{
    ...
    obj->level = atoi(global("maxquest"))<value?atoi(global("maxquest")):value;
    ...
    sprintf(buf,"a %d point quest token",obj->level);
    obj->short_descr = str_dup( buf );
    free_string( obj->description );
    sprintf(buf,"A %d point quest token lies on the floor.",obj->level);
